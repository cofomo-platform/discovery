...
public class IDiscoveryProviderUnitTest {
	...
	public void shouldReturnAllActiveMobilityProvider() throws Exception {
		...
		// action
		this.mockMvc.perform(get("/api/active")).andDo(print()).andExpect(status().isOk())
	...
	public void shouldReturnOneMobilityProvider() throws Exception {
		...
		// action
		this.mockMvc.perform(get("/api/1").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
	...
	public void shouldAddMobilityProvider() throws Exception {
		...
		MvcResult result = this.mockMvc
				.perform(post("/api").content(objectMapper.writeValueAsString(mp))
	...
	public void shouldUpdateOneMobilityProvider() throws Exception {
		...
		// action
		this.mockMvc
				.perform(put("/api/1").content(objectMapper.writeValueAsString(mp))
						.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
	...
	public void shouldDeleteMobilityProvider() throws Exception {
		...
		// action
		this.mockMvc.perform(delete("/api/1")).andExpect(status().isOk()).andDo(document("delete"))
				.andDo(print());
	}
